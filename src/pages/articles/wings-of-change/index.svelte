...
<script>
  ...
  import USOutline from './_data/US_outline.json';
  ...
  if (routify.inBrowser) {
    const callback = async () => {
      const svg1 = d3.select('#mult_1');
      const svg2 = d3.select('#mult_2');
      const svg3 = d3.select('#mult_3');
      const svg4 = d3.select('#mult_4');
      const woodThrushFull = await d3.csv(woodthrushSample, d3.autoType);

      const requestData = (svg, year) => {
        // GeoJson Source: https://eric.clst.org/tech/usgeojson/
        const width = svg.attr('width');
        const height = svg.attr('height');

        const projection = d3.geoMercator().fitSize([width, height], USOutline);
        ...
        const woodThrush = woodThrushFull.filter(d => d.year == year);

        const pos_vals = [];
        woodThrush.forEach(d => {
          d.position = projection([d.lon, d.lat]);
          pos_vals.push(d.position);
        });
        ...
        const contours = d3.contourDensity()
          ...
          .size([width, height])
          .bandwidth(10)
          .thresholds(10)(woodThrush);

        const density_values = [];

        // Contour source: https://observablehq.com/@d3/density-contours
        contours.forEach(d => {
          density_values.push(d.value);
        });

        const myColor = d3.scaleLinear().domain(d3.extent(density_values)).range(['white', '#832232']);
      ...
      requestData(svg1, 2020);
      requestData(svg2, 2010);
      requestData(svg3, 2000);
      requestData(svg4, 1990);
      ...
      const vis_avg_lat_long = async () => {
        ...
        loader([
          { type: 'script', url: 'https://unpkg.com/d3-regression@1.3.4/dist/d3-regression.min.js' }], (() => !!d3.regressionLinear), () => {
...
<div class="wrapper-inner">
  ...
  <div id=mult_box1>
    <div id=box1 class=box>
      <h3>1990</h3>
      <svg id=mult_4 height=210 width=420></svg>
    </div>
    <div id=box2 class=box>
      <h3>2000</h3>
      <svg id=mult_3 height=210 width=420></svg>
    </div>
  </div>
  <div id=mult_box2>
    <div id=box3 class=box>
      <h3>2010</h3>
      <svg id=mult_2 height=210 width=420></svg>
    </div>
    <div id=box4 class=box>
      <h3>2020</h3>
      <svg id=mult_1 height=210 width=420></svg>
    </div>
  </div>
  <br>
  <!--Map-->
